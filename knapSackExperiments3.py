...
def keepItem(i, val_i, cap_i, M):
    ...
    if cap_i in M:
        ...
        M2 = deepcopy(M[i])
        for k in M[i]:
            if k > 0 and cap_i < k and val_i >= M[i][k][1]:
                del M2[k]  # delete item from dict
                flag = True
            elif cap_i >= k and val_i <= M[i][k][1]:
                return False
        if flag:
            M[i] = deepcopy(M2)
            return True
...
if __name__ == "__main__":
    ...
    # M is a list of dicts; 1 dict for each item (row of matrix)
    M = [None] * (number_of_items + 1)  # for each item, including item 0

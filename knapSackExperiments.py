...
#    cap_i = 0
#    for i in range(1, n + 1):
#        for c in M[i - 1]:  # c = (wt, value)
#            if c[0] > W:
#                continue
#            #value = M[i - 1][c]
#            value = c[1]
#            #cap_i = items[i].weight + c
#            cap_i = items[i].weight + c[0]
#            if cap_i > W:
#                break
...
#            else:
#                #M[i][cap_i] = val_i
#                last_item_on_list = M[i][-1]
#                if cap_i <= last_item_on_list[0] and val_i > last_item_on_list[1]:
#                    M[i].pop()
#                M[i].append((cap_i, val_i))
#                # extend to rest of items
#                # extend to rest of items
#                for z in range(i + 1, n + 1):
#                    #M[z][cap_i] = M[i][cap_i]
#                    M[z].append((cap_i, val_i))
...
def keepItem(i, val_i, cap_i, M):
    ...
    """
    prev_item = items[i - 1]
    t = (prev_item.weight, prev_item.value)
    if t in M[i]:
        if val_i > t[1]:
            return True
    return False

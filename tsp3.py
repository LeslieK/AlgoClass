...
This program stores and plots the TSP route.
"""
import math
import numpy as np
...
if __name__ == "__main__":
    ...
    for m in range(2, number_cities + 1):
        for S in subsets(number_cities, m):
            #print(S)
            if S not in A:
                A[S] = [np.inf] * (number_cities + 1)
                penultimate[S] = [None] * (number_cities + 1)
            for j in S:
                # j is endpoint
                if j > 1:
                    zmin = np.inf
    ...
    for k in range(2, number_cities + 1):  
        r = A[all_vertices][k] + distance(cities, k, 1)
        print('dist:', r, A[all_vertices][k], distance(cities, k, 1))
        if r < zmin:
            zmin = r
            kbest = k
    penultimate[all_vertices][1] = kbest
    opt_tour = zmin
